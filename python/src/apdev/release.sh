...
# Auto-detect project name from pyproject.toml (unless set via env)
...
step6_5_create_github_release() {
    ...
    if [ -z "$GITHUB_REPO" ]; then
        echo -e "${YELLOW}⚠️  GitHub repo not detected. Set GITHUB_REPO env variable (e.g. owner/repo)${NC}"
    ...
    if command -v gh &> /dev/null; then
        ...
        if [ "$USE_API" = true ] && [ -n "$GITHUB_TOKEN" ]; then
            ...
            echo -e "${CYAN}   https://github.com/${GITHUB_REPO}/releases/new${NC}"
...
See [CHANGELOG.md](CHANGELOG.md) for details."
            ...
            if [ "$USE_API" = true ] && [ -n "$GITHUB_TOKEN" ]; then
                if command -v jq &> /dev/null; then
                    if create_github_release_api "${TAG}" "Release ${VERSION}" "$NOTES_FILE" "$GITHUB_TOKEN" "${GITHUB_REPO}"; then
...
step_summary() {
    ...
    if [ -n "$GITHUB_REPO" ]; then
        if check_release_exists; then
            ...
            echo -e "     Create at: https://github.com/${GITHUB_REPO}/releases/new"
        else
